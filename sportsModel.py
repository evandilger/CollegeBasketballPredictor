...
def transform_data(teamData, gamesData):
    ...
    for index, row in games_data.iterrows():
        ...
        if (not win.empty) and (not lose.empty):
            ...
            winRow = pd.merge(left=winRow,right=combinedWin, on='joincol', how='outer')
            ...
            loseRow = pd.merge(left=loseRow,right=combinedLose, on='joincol', how='outer')
